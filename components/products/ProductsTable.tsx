      ...
      <table className="min-w-full bg-white">
        <thead className="bg-white">
          <tr>
            ...
            <th className="py-2 px-4 border-b text-black text-left">Categoría</th>
            ...
            <th className="py-2 px-4 border-b text-black text-left">Acciones</th>
          </tr>
        </thead>
        <tbody>
          {productList.map((product) => (
            <tr key={product.id}>
              ...
              <td className="py-2 px-4 text-black font-bold border-b">${product.price}</td>
              <td className="py-2 px-4 text-black font-bold border-b">
                {product.category?.name || "Sin categoría"}
              </td>
              <td className="py-2 px-4 font-bold border-b">
                <span className={`${product.stock > 5 ? "text-green-600" : product.stock > 0 ? "text-yellow-600" : "text-red-600"} font-medium`}>
                  {product.stock}
                </span>
              </td>
              <td className="py-2 px-4 border-b">
                <span className={`px-2 py-1 rounded ${product.isActive ? "bg-green-200 text-green-800" : "bg-red-200 text-red-800"}`}>
              ...
              <td className="py-2 px-4 border-b">
                <div className="flex gap-2">
                  <button
                    ...
                    className={`px-3 py-1 rounded ${product.isActive ? "bg-red-100 text-red-600 hover:bg-red-200" : "bg-green-100 text-green-600 hover:bg-green-200"}`}
                  >
